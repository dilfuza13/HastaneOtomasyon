...
// genel ayarları dahil ediyruz
include('ayarlar.php');

// üye girişi var mı konbtrol ediyoruz
if(!isset($_SESSION['patient']['id'])){
	header("Location: index.php");
	exit;
}
$hastaid = $_SESSION['patient']['id'];

// Hasta bilgilerini çekelim
$hasta = mysqli_fetch_assoc($mysqli->query("SELECT * FROM patient WHERE id='$hastaid'"));
?>
<!doctype html>
<html lang="tr">
<head>
...
<div class="header-gradient shadow-sm d-flex justify-content-between align-items-center">
	<div>
		<h2 class="fw-bold mb-1">Merhaba, <?=$hasta['name'];?>! 👋</h2>
...
<div class="row g-4">
	...
	<div class="main-card p-4 shadow-sm">
		...
		<table class="table align-middle">
			...
			$myQuery = "SELECT a.id as aid, a.timeslot, d.name as dname, s.specialization as kname 
						...
						WHERE a.patient='$hastaid' AND a.timeslot >= CURRENT_TIMESTAMP() ORDER BY a.timeslot ASC";
			...
			if($res->num_rows > 0){
				...
				<tr>
					...
					<td class="text-end">
					<form action="islemler.php" method="post" class="d-inline" onsubmit="return confirm('Bu randevuyu iptal etmek istediğinizden emin misiniz?')">
						<input type="hidden" name="islem" value="randevuiptalet">
	...
	<div class="main-card p-4 shadow-sm">
		...
		<table class="table align-middle">
			...
			$myQuery = "SELECT r.*, d.name as dname, s.specialization as kname FROM requests as r
							...
							WHERE patient='$hastaid' ORDER BY createdtime DESC";
			$res = $mysqli->query($myQuery);
			if($res->num_rows > 0){
				while($rs = mysqli_fetch_array($res)){ ?>
				<tr>
	...
	<div class="main-card p-4 shadow-sm">
		...
		<table class="table align-middle">
			...
			$myQuery = "SELECT * FROM uploads WHERE patient='$hastaid' ORDER BY createdtime DESC";
			$res = $mysqli->query($myQuery);
			if($res->num_rows > 0){
				while($rs = mysqli_fetch_array($res)){ ?>
				<tr>
